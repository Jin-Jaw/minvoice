// Gmail payment confirmations: read one mailbox (read-only scope) for trusted
// payment emails and close the sent invoice each one clearly belongs to.
// Ambiguous messages are recorded for review and never change an invoice.
import type { Bindings } from '../env';
import { getSettings, type Invoice } from '../db/queries';
import { secretConfigured, trustedSenderQuery } from '../lib/config';
import { unbox } from '../lib/secretbox';
import { notifyInvoicePaid } from './telegram/notifications';
...
export async function scanGmailPayments(env: Bindings): Promise<GmailScanResult> {
  ...
  for (const item of list.messages ?? []) {
    ...
    result[outcome] += 1;
    if (outcome === 'paid' && match.kind === 'paid') {
      // Same Telegram message as a manual "mark paid"; a failure must not stop the scan.
      await notifyInvoicePaid(env, match.invoice.id).catch((error) =>
        console.error(JSON.stringify({ event: 'gmail_paid_notification_failed', invoiceId: match.invoice.id, error: String(error) }))
      );
    }
  }
