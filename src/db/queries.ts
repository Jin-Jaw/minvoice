...
export type Settings = {
  ...
  // Gmail payment confirmations (shared across companies; see services/gmail-payments)
...
/** Store a freshly granted Gmail connection (refresh token already sealed) and switch matching on. */
...
/** Forget the Gmail account. The search query is kept for a later reconnect. */
...
export async function monthlyReport(
  ...
  const [inv, pay, expense, income] = await db.batch<{ ym: string; currency: string; n: number; total: number }>([
    ...
    db.prepare(
      `SELECT strftime('%Y-%m', ie.income_date) AS ym, ie.currency, COUNT(*) AS n,
              COALESCE(SUM(ie.amount_cents), 0) AS total
       FROM income_entries ie JOIN branches b ON b.id = ie.branch_id
       WHERE b.workspace_id = ?3 AND (?1 IS NULL OR ie.branch_id = ?1) AND ie.voided_at IS NULL
         AND (?2 IS NULL OR ie.client_id = ?2)
       GROUP BY ym, ie.currency`
    ).bind(branchId, clientId, workspaceId),
  ]);
...
export async function reportSummary(
  ...
  const [counts, outstanding, received, expenses, income] = await db.batch([
    ...
    db
      .prepare(
        `SELECT ie.currency, COALESCE(SUM(ie.amount_cents), 0) AS cents
         FROM income_entries ie JOIN branches b ON b.id = ie.branch_id
         WHERE b.workspace_id = ?4 AND (?1 IS NULL OR ie.branch_id = ?1) AND ie.voided_at IS NULL
           AND strftime('%Y', ie.income_date) = substr(?2, 1, 4)
           AND (?3 IS NULL OR ie.client_id = ?3) GROUP BY ie.currency`
      )
      .bind(branchId, today, clientId, workspaceId),
  ]);
