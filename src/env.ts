export type Bindings = {
  ...
  *  do not emulate Rate Limiting bindings can still start. */
  ...
  GMAIL_CLIENT_ID?: string; // Google OAuth web client used for read-only Gmail access
  GMAIL_CLIENT_SECRET?: string;
  SETTINGS_MASTER_KEY?: string; // encrypts credentials stored in Settings (D1); generated by `npm run deploy`
  ADMIN_PASSWORD?: string; // fallback admin login until Cloudflare Access is configured
};

export type AppEnv = {
  Bindings: Bindings;
  Variables: { secureHeadersNonce?: string; branchId: number; branchName: string };
};
